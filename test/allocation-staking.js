...
describe("AllocationStaking", function () {
    ...
    async function baseSetup(params) {
        await ZSNToken.approve(AllocationStaking.address, TOKENS_TO_ADD);
    ...
    beforeEach(async function () {
        ...
        const BreTokenFactory = await ethers.getContractFactory("ZSNToken");
        ZSNToken = await BreTokenFactory.deploy("ZSN", "ZSN", ethers.utils.parseUnits("1000000000"), 18);
        ...
        await AllocationStaking.initialize(ZSNToken.address, REWARDS_PER_SECOND, startTimestamp, SalesFactory.address);
    ...
    context("Setup", async function () {
        it("Should setup the token correctly", async function () {
            // When
            let decimals = await ZSNToken.decimals();
            let totalSupply = await ZSNToken.totalSupply();
            let deployerBalance = await ZSNToken.balanceOf(deployer.address);
        ...
        it("Should add a pool successfully", async function () {
            // When
            await AllocationStaking.add(ALLOC_POINT, ZSNToken.address, false);
        ...
        it("Should add a pool successfully with mass update", async function () {
            // When
            await AllocationStaking.add(ALLOC_POINT, ZSNToken.address, true);
    ...
    context("Fund", async function () {
        it("Should fund the farm successfully", async function () {
            // Given
            let deployerBalanceBefore = await ZSNToken.balanceOf(deployer.address);
            let rewardPerSecond = await AllocationStaking.rewardPerSecond();
            ...
            // Then
            let deployerBalanceAfter = await ZSNToken.balanceOf(deployer.address);
            let contractBalanceAfter = await ZSNToken.balanceOf(AllocationStaking.address);
        ...
        it("Should not fund the farm if reward per second is 0", async function () {
            ...
            const blockTimestamp = await getCurrentBlockTimestamp();
            AllocationStaking = await AllocationStakingRewardsFactory.deploy();
            AllocationStaking.initialize(ZSNToken.address, 0, blockTimestamp + START_TIMESTAMP_DELTA, SalesFactory.address);
            await ZSNToken.approve(AllocationStaking.address, TOKENS_TO_ADD);
    ...
    context("Deposits", async function () {
        ...
        describe("Deposit", async function () {
            ...
            xit("Should pay user pending amount before adding new deposit", async function () {
                ...
                const balanceERC20Before = await ZSNToken.balanceOf(deployer.address);
                ...
                const balanceERC20After = await ZSNToken.balanceOf(deployer.address);
