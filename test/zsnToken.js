...
describe("breToken:ERC20", () => {
    ...
    describe("burn", () => {
        it("should not burn more than account balance", async () => {
            ...
            await expect(breToken.burn(oldBalance.add(unitTokenAmount))).to.be.revertedWith(
...
describe("breToken:ERC20:transfer", () => {
    ...
    describe("when the sender is invalid address", () => {
        it("reverts", async () => {
            expect(await isEthException(breToken.transfer(ZERO_ADDRESS, overdraftAmount))).to.be.true;
        });
    });

    describe("when the sender does NOT have enough balance", () => {
        it("reverts", async () => {
            expect(await isEthException(breToken.transfer(recipientAddr, overdraftAmount))).to.be.true;
        });
    });
    ...
    describe("when the recipient is the zero address", () => {
        it("should fail", async () => {
            expect(await isEthException(breToken.transfer(ZERO_ADDRESS, transferAmount))).to.be.true;
        });
    });
});
    ...
    describe("when the spender does NOT have enough approved balance", () => {
            ...
            it("reverts", async () => {
                ...
                expect(
                    await isEthException(
                        breToken.connect(anotherAccount).transferFrom(ownerAddr, recipientAddr, overdraftAmount)
                    )
                ).to.be.true;
            });
        });
        ...
        describe("when the owner has enough balance", () => {
            it("reverts", async () => {
                ...
                expect(
                    await isEthException(
                        breToken.connect(anotherAccount).transferFrom(ownerAddr, recipientAddr, transferAmount)
                    )
                ).to.be.true;
            });
        });
    ...
    describe("when the spender has enough approved balance", () => {
        ...
        describe("when the owner has enough balance", () => {
            ...
            before(async () => {
                ...
                r = await (
                    await breToken.connect(anotherAccount).transferFrom(ownerAddr, recipientAddr, transferAmount)
                ).wait();
            });
        ...
        describe("when the sender has enough balance", () => {
            describe("when there was no approved amount before", () => {
                ...
                it("should not decrease allowance more than current allowance", async () => {
                    ...
                    await expect(
                        breToken.decreaseAllowance(anotherAccountAddr, currentAllowance.add(unitTokenAmount))
