...
async function main() {
    ...
    const zsnTokenAddress = getSavedContractAddresses()[hre.network.name]["ZSN-TOKEN"];
    console.log("zsnTokenAddress: ", zsnTokenAddress);

    const farm = await hre.ethers.getContractFactory("FarmingZSN");
    const Farm = await farm.deploy(zsnTokenAddress, ethers.utils.parseEther(RPS), startTS);
    await Farm.deployed();
    ...
    const approveTx = await ZSN.approve(Farm.address, ethers.utils.parseEther("50000"));
    await approveTx.wait();

    let tx = await Farm.fund(ethers.utils.parseEther("50000"));
    await tx.wait();
    // add lp token
    const lpTokenAddress = getSavedContractAddresses()[hre.network.name]["ZSN-TOKEN"];
